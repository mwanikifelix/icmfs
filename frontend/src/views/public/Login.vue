...
import bgImage from '@/assets/media/logo.png';
...
async function handleLogin() {
    ...
    try {
        ...
        if (err.response?.status === 401) {
            ...
            error.value = messages[0] || 'Login failed. Please try again.';
        } else {
...
<template>
    ...
    <!-- ✅ BACKGROUND IMAGE APPLIED HERE -->
        <div
        class="bg-surface-50 dark:bg-surface-950 flex items-center justify-center min-h-screen min-w-[100vw] overflow-hidden animated-bg"
        :style="{
            backgroundImage: `linear-gradient(rgba(13,35,71,0.8), rgba(25,118,210,0.8)), url(${bgImage})`
        }"
        >
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                ...
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    ...
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">ICMFS</div>
                    ...
                    <div>
                        ...
                        <InputText v-model="username" class="w-full mb-8" />

                        <label class="block text-xl mb-2">Password</label>
                        <Password v-model="password" toggleMask fluid :feedback="false" class="mb-4" />
...
.animated-bg {
    ...
    animation: floatBackground 20s ease-in-out infinite,
               zoomBg 30s ease-in-out infinite;
}
